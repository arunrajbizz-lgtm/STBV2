...
const Player = ({ 
  ...
  const nowPlaying = isLive ? epg.find(p => nowTs >= p.start_timestamp && nowTs < p.stop_timestamp) : null;
  const nextProgram = isLive ? epg.find(p => p.start_timestamp > nowTs) : null;
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (menuMode) {
      ...
      if (menuMode === 'audio' || menuMode === 'subtitle') {
         ...
         if (key === 38) setFocusedMenuIdx(p => Math.max(0, p - 1));
         if (key === 40) setFocusedMenuIdx(p => Math.min(channels.length - 1, p + 1));
         if (key === 13) {
            const target = channels[focusedMenuIdx];
            if (target && target.id !== item.id) {
               window.dispatchEvent(new CustomEvent('tune_channel', { detail: target }));
      ...
      if (key === 10009 || key === 27 || key === 405) setMenuMenuMode(null); // Back, Esc, or Yellow closes the menu
      return;
    }
  ...
  }, [menuMode, showHud, isLive, onNext, onPrev, item, isFavorite, handleAction, resetHudTimer, resetMenuTimer, audioTracks, focusedMenuIdx, channels, subtitleTracks, activeHudZone, isPlaying, focusedControlIdx]);
  ...
  return (
    ...
    >
      ...
      {menuMode === 'miniguide' && (
         <div className="absolute inset-x-0 bottom-0 z-[500] animate-in slide-in-from-bottom duration-500 bg-black/80 backdrop-blur-3xl border-t border-white/5 p-8">
            <div className="max-w-[1800px] mx-auto flex gap-4 overflow-hidden">
               <div className="flex gap-4 transition-transform duration-500 py-2">
                  {visibleChannels.map((ch, index) => {
                     ...
                     return (
                        <div 
                           key={ch.id} 
                           className={clsx(
                              "w-[320px] p-4 rounded-2xl flex items-center gap-4 transition-all border-2 shrink-0",
                              isFocused ? "bg-white text-black border-white scale-110 shadow-focus z-10" : (isActive ? "bg-accent/10 border-accent/30 text-accent" : "bg-white/5 text-white/30 border-transparent")
                           )}
                        >
                           <div className="w-12 h-12 glass rounded-lg flex items-center justify-center p-1.5 border border-white/5">
                              <img src={getLogoUrl(ch)} className="w-full h-full object-contain" alt="" onError={(e)=>e.target.src='https://via.placeholder.com/64x64/111/444?text=CH'} />
                           </div>
                           <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                 <span className="font-black text-[10px] opacity-40">CH {ch.number}</span>
                                 <span className="font-black italic uppercase truncate text-sm">{ch.name}</span>
                              </div>
                              <p className="text-[10px] font-bold truncate opacity-50 uppercase tracking-tighter">{ch.cur_playing || 'Live Broadcast'}</p>
                           </div>
                        </div>
                     );
                  })}
      ...
      {/* FULL EPG GUIDE (P28) */}
      {menuMode === 'guide' && (
         <div className="absolute inset-0 bg-black/95 z-[600] flex flex-col p-20 animate-in fade-in duration-300">
            <div className="flex items-center justify-between mb-16">
               <div className="flex items-center gap-8">
                  ...
                  <div>
                     <h1 className="text-6xl font-black italic tracking-tighter uppercase leading-none">Program Guide</h1>
                     <p className="text-2xl font-bold text-white/40 mt-2 uppercase tracking-widest">{activeProvider?.name} • {formatClock()}</p>
                  </div>
               </div>
               
               <div className="flex flex-col items-end text-right">
                  <h3 className="text-3xl font-black text-accent italic uppercase">{channels[focusedMenuIdx]?.name}</h3>
                  <p className="text-xl font-bold text-white/50">{channels[focusedMenuIdx]?.cur_playing || 'Live Broadcast'}</p>
               </div>
            </div>
            ...
            <div className="flex-1 flex gap-12 overflow-hidden">
               ...
               <div className="w-[500px] flex flex-col gap-4 overflow-y-auto pr-4 custom-scrollbar">
                  {channels.map((ch, index) => {
                     const isFocused = focusedMenuIdx === index;
                     const isActive = item.id === ch.id;
                     return (
                        <div key={ch.id} className={clsx("p-6 rounded-3xl flex items-center gap-6 transition-all border-2", isFocused ? "bg-white text-black border-white scale-[1.02] shadow-focus" : (isActive ? "bg-accent/10 border-accent/20 text-accent" : "bg-white/5 text-white/30 border-transparent"))}>
                           <span className="w-14 font-black text-lg opacity-40">{ch.number}</span>
                           ...
                           <span className="flex-1 font-black italic uppercase truncate text-xl">{ch.name}</span>
                        </div>
                     );
                  })}
               </div>
               ...
               <div className="flex-1 flex flex-col gap-8">
                  <div className="glass p-12 rounded-[50px] border border-white/10 flex-1 flex flex-col">
                     <p className="text-accent font-black tracking-[10px] text-xs mb-4 uppercase">PROGRAM OVERVIEW</p>
                     <h2 className="text-5xl font-black italic tracking-tighter uppercase mb-6 text-white leading-tight">
                        {channels[focusedMenuIdx]?.cur_playing || 'Live Broadcast'}
                     </h2>
                     <div className="flex items-center gap-6 text-2xl font-bold text-white/40 mb-10 italic">
                        <Clock size={24} /> 
                        {channels[focusedMenuIdx]?.cur_playing ? `${formatTimeStr(nowTs)} - ${formatTimeStr(nowTs + 3600)}` : 'Continuous Broadcast'}
                     </div>
                     <p className="text-2xl text-white/60 leading-relaxed font-medium">
                        Watch high-definition streaming of {channels[focusedMenuIdx]?.name}. Enjoy premium content, breaking news, and trending entertainment directly from the portal.
                     </p>
                     
                     <div className="mt-auto pt-10 border-t border-white/5">
                        <p className="text-white/20 font-black tracking-widest text-sm mb-4 uppercase">COMING UP NEXT</p>
                        <div className="flex items-center justify-between">
                           <span className="text-3xl font-bold text-white/80 italic">{channels[focusedMenuIdx]?.epg?.[1]?.name || 'Next Program'}</span>
                           <span className="text-2xl font-mono text-white/30 italic">22:00</span>
                        </div>
                     </div>
                  </div>
               </div>
            ...
            {/* COLOR LEGEND BOTTOM */}
            <div className="flex justify-center gap-12 mt-16 pt-10 border-t border-white/5">
